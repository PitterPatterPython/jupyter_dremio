...
from dremio_core._version import __desc__
...
class Dremio(Pyodbc):
    ...
    custom_evars = ["dremio_conn_default"]
    ...
    custom_allowed_set_opts = ["dremio_conn_default"]



    # These are the custom options for your integration
    myopts = {}
    myopts['dremio_max_rows'] = [1000, 'Max number of rows to return, will potentially add this to queries']

    myopts['dremio_conn_default'] = ["default", 'Default instance name for connections']

    # Class Init function - Obtain a reference to the get_ipython()

    def __init__(self, shell, debug=False, *args, **kwargs):
        super(Dremio, self).__init__(shell, debug=debug)
        self.debug = debug
    ...
    @line_cell_magic
    def dremio(self, line, cell=None):
        if cell is None:
            line = line.replace("\r", "")
